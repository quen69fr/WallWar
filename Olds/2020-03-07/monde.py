...
class Monde:
    ...
    def cree_personne(self, type_personne, x_carte, y_carte, objectif=None):
        if DIC_ELEMENTS[PARAM_F_PERSONNE_TYPE_CLASS][type_personne] == TYPE_PERSONNE_CLASS_PERSONNE:
            self.add_personne(Personne(type_personne, self.carte, x_carte, y_carte))
            return True
        elif DIC_ELEMENTS[PARAM_F_PERSONNE_TYPE_CLASS][type_personne] == TYPE_PERSONNE_CLASS_PORTEUR:
            self.add_personne(Porteur(type_personne, self.carte, x_carte, y_carte, objectif=objectif))
            return True
        elif DIC_ELEMENTS[PARAM_F_PERSONNE_TYPE_CLASS][type_personne] == TYPE_PERSONNE_CLASS_SOLDAT:
            self.add_personne(Soldat(type_personne, self.carte, x_carte, y_carte, objectif=objectif))
            return True
        return False

    def add_personne(self, personne: ElementMobile):
        self.liste_personnes.append(personne)

    def remove_personne(self, personne: ElementMobile):
    ...
    def update_batiments_constuits(self):
        for batiment in self.liste_batiments_constuits:
            ...
            if type_construction_prete is not None and \
                    len(self.liste_personnes) < Batiment.nb_personne_max:
