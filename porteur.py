...
class Porteur(Personne):
    ...
    def update_ecran_original(self):
        ...
        if not self.ressource_comptenu == 0:
            ...
            x = int(x / 2 + Element.dic_elements[PARAM_F_PORTEUR_DISTANCE_RESSOURCE][self.type])
            y = int(y / 2)
            draw_filled_circle(self.ecran_original, (x, y),
                               Element.dic_elements[PARAM_F_PORTEUR_RAYON_RESSOURCE][self.type],
    ...
    @property
    def ressource_comptenu_max(self):
